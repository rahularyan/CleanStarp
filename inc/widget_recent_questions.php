<?php
	class cs_widget_recent_questions {
...
var_dump($questions);
			...
			foreach ($questions as $post){
				...
				$themeobject->output('<li><span class=""></span><a href="'. qa_q_path_html($post['postid'], $post['title']) .'">'.cs_truncate($post['title'], 50).'<span class="time">'.implode(' ', $when).'</span><span class="ans-count total-'.$post['acount'].'">'.$post['acount'].'</span></a></li>');
			}
