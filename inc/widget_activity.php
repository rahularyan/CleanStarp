		...
		function list_events($query, $events_type, $limit) {
			...
			while ( ($row = qa_db_read_one_assoc($query,true)) !== null ) {
				if(in_array($row['event'], $events_type)) {
					...
					if($postid !== null) {
						$getPostType = cs_get_cache("SELECT type,parentid FROM `^posts` WHERE `postid` = #", $postid); //mysql_fetch_array( qa_db_query_sub("SELECT type,parentid FROM `^posts` WHERE `postid` = #", $postid) );
						$postType = $getPostType[0]; // type, and $getPostType[1] is parentid
						if($postType=="A") {
							$getQtitle = cs_get_cache("SELECT title FROM `^posts` WHERE `postid` = # LIMIT 1", $getPostType[1]);// mysql_fetch_array( qa_db_query_sub("SELECT title FROM `^posts` WHERE `postid` = # LIMIT 1", $getPostType[1]) );
							...
							$getQlink = cs_get_cache("SELECT parentid,type FROM `^posts` WHERE `postid` = # LIMIT 1", $getPostType[1]); //mysql_fetch_array( qa_db_query_sub("SELECT parentid,type FROM `^posts` WHERE `postid` = # LIMIT 1", $getPostType[1]) );
							$linkToQuestion = $getQlink[0];
							if($getQlink[1]=="A") {
								$getQtitle = cs_get_cache("SELECT title FROM `^posts` WHERE `postid` = # LIMIT 1", $getQlink[0]); //mysql_fetch_array( qa_db_query_sub("SELECT title FROM `^posts` WHERE `postid` = # LIMIT 1", $getQlink[0]) );
								$qTitle = (isset($getQtitle[0])) ? $getQtitle[0] : "";
								// get correct public URL
						...
						// if question is hidden, do not show frontend!
							...
							$getQtitle = cs_get_cache("SELECT title FROM `^posts` WHERE `postid` = # LIMIT 1", $postid); //mysql_fetch_array( qa_db_query_sub("SELECT title FROM `^posts` WHERE `postid` = # LIMIT 1", $postid) );
							$qTitle = (isset($getQtitle[0])) ? $getQtitle[0] : "";
							// get correct public URL
							// $activity_url = qa_path_html(qa_q_request($getPostType[1], $qTitle), null, qa_opt('site_url'), null, null);
